...
export async function POST(request: Request) {
  try {
    ...
    if (!componentString) {
      ...
      return new Response(JSON.stringify({ error: 'No component string provided' }), {
        status: 400,
    ...
    if (!componentString.includes('"use client"')) {
      return new Response(JSON.stringify({ error: "Component must start with 'use client'" }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(JSON.stringify({ message: 'Component updated successfully!' }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('❌ Error writing component file:', error);
    return new Response(JSON.stringify({ error: 'Failed to update component' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
